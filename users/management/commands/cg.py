        ...
        for perm in permissions:
            try:
                ...
                permission = Permission.objects.get(
                    codename=perm["codename"], content_type=content_type
                )
                manager_group.permissions.add(permission)
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Право "{permission.name}" добавлено в группу "manager"'
                    )
                )
            except Permission.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(
                        f'Право с codename {perm["codename"]} и content_type {content_type} не найдено'
                    )
                )

        self.stdout.write(
            self.style.SUCCESS("Группа manager успешно создана и права назначены")
        )
